...
#> date = "2024-10-28"
#> tags = ["pressure relief", "gas flow", "equations of state"]
#> description = "Sizing pressure relief devices using equations of state."
#> 
#>     [[frontmatter.author]]
#>     name = "Allan Farrell"
...
# ╔═╡ 271a9f9d-e00b-4f63-af3c-90f1943743b8
using PlutoUI

# ╔═╡ 552358c7-8600-4a8f-abdc-5a5732afa9af
using Unitful
...
# ╔═╡ fd2dc586-914c-11ef-2215-2fe1780a6dc2
md"""
...
# ╔═╡ c710a4e1-b947-48ef-8676-dc70a3bfb2bc
md"""
## Sizing a Pressure Relief Valve

The general idea for sizing a relief valve is to determine the minimum area required such that the mass flow through the valve equals the mass flow required for the governing release case via the relation[^2]
...
[^2]: *[API 520](#API-2020)*, equation B.5

"""

# ╔═╡ 0983051f-c2c1-468a-b6ce-56be5fdef63e
md"""
![pressure relief](https://aefarrell.github.io/images/relief_valve_sizing_files/pressure_relief.png)

> A hypothetical pressure relief device, connected to a pressure reservoir (1) and discharging into the atmosphere (2).

"""

# ╔═╡ efebb120-89f4-4337-8cb0-94326c04d904
md"""
...
# ╔═╡ 0b16619b-11da-4939-9fc4-1404b39dcd5c
md"""
### Choked Flow
...
# ╔═╡ 5ac97d9c-54c9-482f-ac0e-c8823c0a6fd9
begin
...
# ╔═╡ 980b2b3a-ddc1-4d66-8748-b37bf90081fe
md"""
...
Considering the choked flow case, we know that $G_t = \frac{c_t}{v_t}$ and, for an ideal gas, the sonic velocity is given by[^3]
...
It can be shown that, along an isentropic path defined by $P v^k = \mathrm{const}$, the critical pressure ratio is[^4]
...
[^3]: [Tilton](#tilton-2008), "Fluid and Particle Dynamics," equation 6-113.
[^4]: [Tilton](#tilton-2008), "Fluid and Particle Dynamics," equation 6-119.
[^5]: *[API 520](#API-2020)*, equation B.21
...
# ╔═╡ 39658d22-19a4-443a-91fc-0a7537a25f33
nozzle_pressure_ideal(P, T, k) = P*(2/(k+1))^(k/(k-1))

# ╔═╡ dfb08499-ec6c-4a4d-8d3d-e3c6d7046db1
nozzle_temperature_ideal(P, T, k) = T*(2/(k+1))

# ╔═╡ b6817ebb-1324-4120-a512-b29a16339cc5
md"""
...
md"""
The derivation of $n$ follows from the definition of the speed of sound in a gas[^7]

```math
c = \sqrt{ \left( {\partial P} \over {\partial \rho} \right)_S} =\sqrt{ -v^2 \left( {\partial P} \over {\partial v} \right)_S}
```

The constant entropy partial derivative can be re-written to eliminate entropy[^8]
...
equating this to the ideal gas case, $c = \sqrt{n P v}$, and solving for $n$ gives[^10]
...
[^7]: [Tilton](#tilton-2008), "Fluid and Particle Dynamics," 6-22; [Gmehling *et al.*](#gmehling-2012), *Chemical Thermodynamics*, 113
[^8]: [Gmehling *et al.*](#gmehling-2012), *Chemical Thermodynamics*, 660
[^9]: [Gmehling *et al.*](#gmehling-2012), *Chemical Thermodynamics*, equations C.21 and C.8 (respectively)
[^10]: *[API 520](#API-2020)*, equation B.13
...
k = isentropic_expansion_factor(ig_ethane, P₁, T₁)

# ╔═╡ 3fd9d664-fe07-4bd3-9091-7fc712090c0e
md"""
The pressure at the nozzle is $(round(typeof(1u"bar"), nozzle_pressure_ideal(P₁,T₁,k))) the temperature at the nozzle is $(round(typeof(1u"K"), nozzle_temperature_ideal(P₁,T₁,k))), which is above the critical point. The fluid super-critical and choked when leaving the PSV.
"""
...
let
	Pc = ustrip(u"bar", vtpr_ethane.params.Pc[1]*1u"Pa")
	Ps = range(1, 200, 100)*1u"bar"
	ks = isentropic_expansion_factor.(ig_ethane, Ps, T₁)
	vtpr_ns = upreferred.(isentropic_expansion_factor.(vtpr_ethane, Ps, T₁))
	gerg_ns = upreferred.(isentropic_expansion_factor.(gerg_ethane, Ps, T₁))
	p1 = plot(Ps, ks, label="Ideal Gas")
	plot!(p1, Ps, vtpr_ns, label="VTPR")
	plot!(p1, Ps, gerg_ns, label="GERG-2008")
	title!(p1, "Isentropic Expansion Factors for Ethane, at $(T₁)")
	xlims!(p1, 0, 200)
	xlabel!(p1, "Stagnation Pressure, P₁ (bar)")
	ylims!(p1, 0.0, 2.5)
	ylabel!(p1, "Isentropic Expansion Factor")
end
...
G_t = \rho_t c_t
```
[^11]: [Crowl *et al*](#crowl-2008), "Process Safety," 23-55; [Gmehling *et al.*](#gmehling-2012), *Chemical Thermodynamics*, 603.
"""
...
let
	...
	Vₗs, Vᵥs = [], []
	for p ∈ Ps
		T, Vₗ, Vᵥ, _ = bubble_temperature(gerg_ethane, p, [1.0])
		append!(Vₗs, Vₗ)
		append!(Vᵥs, Vᵥ)
	end
	plot!(p2, [Vc]./MW, [Pc].*1e-5, label="critical point", color=:black, seriestype=:scatter)
	plot!(p2, Vₗs./MW, Ps.*1e-5, label="phase boundary (GERG-2008)", color=:black, lineweight=2)
	plot!(p2, Vᵥs./MW, Ps.*1e-5, label=:none, color=:black, lineweight=2)

	# remove units from initial conditions
	P, T = ustrip(u"Pa", P₁), ustrip(u"K", T₁)
	V = volume(gerg_ethane,P,T,[1.0])

	# ideal gas path
	k_ig = isentropic_expansion_factor(ig_ethane,P,T)
	Po, To = nozzle_pressure_ideal(P, T, k_ig), nozzle_temperature_ideal(P, T, k_ig)
	P_ig = collect(LinRange(Po, P, 100))
	V1 = volume(ig_ethane,P,T)
	vig(P_t) = V1*(P/P_t)^(1/k_ig)
	V_ig = vig.(P_ig)
	plot!(p2, V_ig./MW, P_ig.*1e-5, label="ideal gas", color=:red, linestyle=:dash)
	plot!(p2, [V_ig[1], V_ig[end]]./MW, [P_ig[1], P_ig[end]].*1e-5, label=:none, color=:red, seriestype=:scatter)

	# isentropic expansion factor
	V1 = volume(gerg_ethane,P,T,[1.0])
	n = isentropic_expansion_factor(gerg_ethane, P, T; z=[1.0])
	Po, To = nozzle_pressure_ideal(P, T, n), nozzle_temperature_ideal(P, T, n)
	P_ief = collect(LinRange(Po, P, 100))
	vief(P_t) = V1*(P/P_t)^(1/n)
	V_ief = vief.(P_ief)
	plot!(p2, V_ief./MW, P_ief.*1e-5, label="isentropic expansion factor", color=:red, linestyle=:dashdot)
	...
	plot!(p2, [Vₜ, V]./MW, [Pₜ, P].*1e-5, label=:none, seriestype=:scatter, color=:red)

	plot!(p2, legend=:topright)
	xlims!(p2, 0, 0.016)
	xlabel!(p2, "Specific Volume, m³/kg")
	ylims!(p2, 0, 250)
	ylabel!(p2, "Pressure, bar")
end
...
let
	...
	categories = ["ideal gas", "ideal gas, real Z", "n factor", "n factor, avg. conditions", "first law balance", "direct integration"]
	models = [("Redlich-Kwong", RK(["ethane"]; idealmodel=ReidIdeal)),
		("Peng-Robinson", PR(["ethane"]; idealmodel=ReidIdeal)),
		("Soave-Redlich-Kwong", SRK(["ethane"]; idealmodel=ReidIdeal)),
		("Volume Translated Peng-Robinson", vtpr_ethane),
		("GERG-2008", gerg_ethane)]
	results = []

	for (name, model) ∈ models
		Z1 = compressibility_factor(model, P₁, T₁)
		ig = mass_flux_choked(ig_ethane, P₁, T₁)
		igrz = ig/√(Z1)
		nf = mass_flux_choked(model, P₁, T₁)

		T_avg = 0.5*(T₁ + nozzle_temperature_ideal(P₁, T₁, k))
		P_avg = 0.5*(P₁ + nozzle_pressure_ideal(P₁, T₁, k))
		n_avg = isentropic_expansion_factor(model, P_avg, T_avg)
		ρ = mass_density(model, P₁, T₁)
		Gₜ² = n_avg*P₁*ρ*(2/(n_avg+1))^((n_avg+1)/(n_avg-1))
		nf_avg = √(Gₜ²)
		
		eb = mass_flux_choked_energy_balance(model, P₁, T₁)
		di = mass_flux_direct_integration(model, P₁, T₁, P₂)
		plot!(p4, categories, [ig, igrz, nf, nf_avg, eb, di], seriestype=:scatter, label=name)
	end

	plot!(p4, xrotation=90, size=(600,650), legend=:right)
	ylims!(3e4, 6e4)
	ylabel!("Theoretical Mass Flux, kg/m²/s")
end
