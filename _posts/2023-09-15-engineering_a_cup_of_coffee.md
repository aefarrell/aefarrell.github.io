...
Making coffee involves heat, mass, and momentum transfer across multiple phases -- pretty standard stuff for undergraduate chemical engineering curricula. On the other hand, while industrial scale leaching operations are generally designed for maximum efficiency -- removing the most amount of a substance with the least amount of solvent, energy, etc. -- coffee makers are specifically designed to avoid that outcome. A saturated cup of coffee would be strong, harsh, and undrinkable. Coffee making, as a unit op, aims at a managed inefficiency, which makes for an interesting design case<a href="#fn-1" class="sidenote-number"></a><span class="sidenote" id="fn-1"> I am not the first person to think of this: using coffee as a basis for exploring engineering concepts is the entire premise of [this book](https://www.amazon.ca/gp/product/B09FSCDY18/). From the reviews it sounds like it is, essentially, a lab manual for exploring chemical engineering concepts using coffee.</span>.
...
<aside class="sidenote" id="fn-2">{{ footnote-2 | markdownify }}</aside>
...
The standard approach to solving this problem is to look at the limiting cases, where the [Biot number](https://en.wikipedia.org/wiki/Biot_number#Mass_transfer_analogue) is either very large or very small<a href="#fn-9" class="sidenote-number"></a><span class="sidenote" id="fn-9">[Seader, Henley and Roper](#seader-2011), *Separation Process Principles*, 663.</span>

+ *Bi* &lt; 0.001 : the mass transfer through the film dominates, a simple exponential model is appropriate
+ 0.001 &lt; *Bi* &lt; 200 : use an intermediate method<a href="#fn-10" class="sidenote-number"></a><span class="sidenote" id="fn-10">The intermediate solution is not given in [Seader](#seader-2011), only a reference: Schwartzberg, Henry G. and R. Y. Chao. 1982. "Solute Diffusivities in Leaching Processes." *Food Technology*. 36, no. 2: 73-86, which has not been digitized and is not available from my local library, so I have no idea what it says ¯\\_(ツ)\_/¯</span>
...
I will argue in a very hand-wavy way that the Biot number for mass transfer is likely to be large, and so the model from [Carslaw and Jaeger](#carslaw-1959)<a href="#fn-11" class="sidenote-number"></a><span class="sidenote" id="fn-11">[Carslaw and Jaeger](#carslaw-1959), *Conduction of Heat in Solids*, 240-241.</span> is the probably the best model. First let's start with Biot number for mass transfer, which for this situation is<a href="#fn-12" class="sidenote-number"></a><span class="sidenote" id="fn-12">[Seader, Henley and Roper](#seader-2011), *Separation Process Principles*, 663.</span>
...
Defining the Biot number in terms of the Sherwood number might, at first glance, not seem tremendously useful. However, if we suppose the Froessling equation<a name="fnref-13" href="#fn-13" class="sidenote-number"></a><span class="sidenote" id="fn-13">[Hottel et al](#hottel-2008), "Heat and Mass Transfer," 5-69.</span> for flow past a single sphere applies
...
Where ${\mathscr{D}_s \over \mathscr{D}_l}$ = 0.1 is assumed from Schwartzberg<a href="#fn-14" class="sidenote-number"></a><span class="sidenote" id="fn-14">[Schwartzberg](#schwartzberg-1987), "Leaching -- Organic Materials," 557.</span>. The Schmidt number, *Sc*, and equilibrium constant, *K*, can be calculated
...
This is the equivalent PDE (in dimensionless form) to the heat transfer case for a hot solid sphere cooling in a well mixed fluid<a href="#fn-15" class="sidenote-number"></a><span class="sidenote" id="fn-15">[Carslaw and Jaeger](#carslaw-1959), *Conduction of Heat in Solids*, 240-241; [Bird, Stewart and Lightfoot](#bird-2007), *Transport Phenomena*, 379-381.</span>, with the solution
